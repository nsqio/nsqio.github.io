...
* **Rate:** The per-second rate of new messages (available when graphite integration is enabled).
...
* **NSQd Host:** Address of the nsqd node connected to.
* **In-flight:** Number of messages sent on a connection which are not yet Finished or Requeued.
...
* **Finished:** Sum of Finish (`FIN`) responses.
* **Requeued:** Sum of Requeue (`REQ`) responses.
* **Messages:** Count of messages sent to this connection.
